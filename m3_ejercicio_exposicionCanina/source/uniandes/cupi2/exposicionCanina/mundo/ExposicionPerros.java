...
public class ExposicionPerros
{
    ...
    public void ordenarPorRaza( )
    {
    	...
    	for (int i=0; i<perros.size()-1; i++  )
    	{
    		...
    		for(int j=1; j<perros.size()-i; j++)
    		{
    			Perro actual=(Perro)perros.get(j);
    			Perro siguiente=(Perro)perros.get(j-1);
    			String razaPerroAct = actual.darRaza();
    			String razaPerroSgte = siguiente.darRaza();
    			if(razaPerroAct.compareToIgnoreCase(razaPerroSgte)<0)
    			{
    ...
    public void ordenarPorNombre( )
    {
        ...
        for(int j = i; j > 0 & !agregado  ; j--){
    			...
    			Perro actual = ( Perro )perros.get( j );
    			Perro siguiente = ( Perro )perros.get( j-1 );
                String nombrePerroActual = actual.darNombre();
                String nombrePerroSiguiente = siguiente.darNombre();
    			if (nombrePerroActual.compareToIgnoreCase(nombrePerroSiguiente)<0) {
    ...
    public void ordenarPorPuntos( )
    {
    	...
    	for (int i=0; i<perros.size()-1; i++ ){
    		...
    		for(int j=1; j<perros.size()-i; j++){
    			Perro actual=(Perro)perros.get(j);
    			Perro siguiente=(Perro)perros.get(j-1);
    			int puntosPerroAct = actual.darPuntos();
    			int puntosPerroSgte = siguiente.darPuntos();
    			if(puntosPerroAct > puntosPerroSgte) {
    ...
    public void ordenarPorEdad( )
    {
    	...
    	for(int i = 0; i < perros.size() - 1; i++) {
    		...
    		Perro PerroMenorEdad = (Perro) perros.get(i);
    		int menorEdad = PerroMenorEdad.darEdad();
    		int posicionMenor = i;
    		for(int j = 1 + i; j < perros.size(); j++) {
    			Perro PerroActualEdad = (Perro) perros.get(j);
        		int actualEdad = PerroActualEdad.darEdad();
    			if(actualEdad < menorEdad) {
    				PerroMenorEdad = PerroActualEdad;
    				posicionMenor = j;
    			}
    			
    		}
    		
    		Perro tem = (Perro) perros.get(i);
    		perros.set(i, PerroMenorEdad);
    		perros.set(posicionMenor, tem);
    	}
    }
